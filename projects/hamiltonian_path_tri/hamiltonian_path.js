...
/*
    This version adapted to arbitrarily shaped sublattices - just have an
    'accept' function Simplified reversal procedure - just go through each
    step (O(N) to reverse, anyway) Different initialisation - start from a
    single point, incrementally add.  Simplified checking of neighbours.
*/
...
//1 - record and draw (show vertices only)
//2 - record and draw (initialise if just turned on)
//3 - draw but no longer record
var showTraceMode = 0;
...
function backbite(n,path)
{
    ...
    var step;
    //switch (Math.floor(Math.random()*4))
    switch (Math.floor(Math.random()*nNeighbours))
    {
        case 0:
            step = [1,0];
            break;
...
function draw_path(n,path)
{
    ...
    if (canvas.getContext)
    {
        ...
        //if ((showTraceMode == 1) || (showTraceMode == 2))
        if (showTraceMode == 1) 
        //draw vertices of trace - won't have a separate function for this for the
        //moment
        {
            //ctx.beginPath();
            //ctx.strokeStyle = "rgba(0,200,255,0.5)"
            //ctx.moveTo((pad+leftTrace[0][0])*sw,(pad+leftTrace[0][1])*sh);
            for (i=0; i<nLeftTrace; i++)
            {
                ctx.beginPath();
                x = (pad+leftTrace[i][0])*sw;
                y = (pad+leftTrace[i][1])*sh;
                //ctx.arc(x,y,0.4*0.5*(sw+sh),0.0,2.0*Math.PI,true);
                ctx.arc(x,y,0.15*Math.min(sw,sh),0.0,2.0*Math.PI,true);
                ctx.fillStyle = "rgb(255,255,255)";
                ctx.closePath();
                ctx.fill();
            }
            for (i=0; i<nRightTrace; i++)
            {
                ctx.beginPath();
                x = (pad+rightTrace[i][0])*sw;
                y = (pad+rightTrace[i][1])*sh;
                //ctx.arc(x,y,0.4*0.5*(sw+sh),0.0,2.0*Math.PI,true);
                ctx.arc(x,y,0.15*Math.min(sw,sh),0.0,2.0*Math.PI,true);
                ctx.fillStyle = "rgb(255,255,255)";
                ctx.closePath();
        ...
        if ((showTraceMode == 2) || (showTraceMode == 3))
...
function refresh_path()
{
    ...
    var is_triangular = document.path_parameters.elements["is_triangular"].checked;
    if (is_triangular)
    {
       nNeighbours = 6;
    }
...
document.addEventListener('keydown', function(event) {
    ...
    else if (((event.keyCode == 57) || (event.keyCode == 105)) && (nNeighbours == 6))
    {
        //NE 9
        step = [1,-1];
        if (left_end)
    ...
    else if (((event.keyCode == 49) || (event.keyCode == 97)) && (nNeighbours == 6))
    {
        //SW 1
        step = [-1,1];
        if (left_end)
